...
const Game = () => {
    ...
    useEffect (() => {
        ...
        app.state.addChild(startScreen);
        app.stage.addChild(playScreen);
        app.state.addChild(endScreen);

        //Set up Title Screen
        let titleRect = new PIXI.Graphics();
        titleRect.beginFill(0xFF0000);
        titleRect.drawRect(0,0,app.view.width, app.view.height);
        startScreen.addChild(titleRect);

        let startText = new PIXI.Text("Welcome to the OG - Use your arrow keys to collect OK's and Avoid !'s");
