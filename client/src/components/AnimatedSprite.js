...
class AnimatedSprite extends PureComponent {
    ...
    constructor(props) {
        ...
        this.positionRef = React.createRef(this.length);
    }

    animate = () => {
        let x;
        this.positionRef.current += this.props.speed ? this.props.speed : 10;
        x = -(this.positionRef.current * 0.6) + this.props.shift;
        x %= this.length * 2;
        if (x < 0) {
            x += this.length * 2;
        }
